...
### Custom Parsers
New parsers must:
- Detect \`\`\`markdown-ui-widget blocks
- Generate unique IDs
- Base64 encode payloads  
- Output standardized XML format

### Custom Renderers
New renderers must:
- Parse XML tags with base64 payloads
- Decode to widget configurations
- Create appropriate UI components
- Emit \`{id, value}\` events on interaction
